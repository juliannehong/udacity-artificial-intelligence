...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set()) # add one more action level
        for a in self.all_actions: # go through all the possible actions
            na = PgNode_a(a) # na.prenodes = set(PgNode_s)
            if na.prenodes.issubset(self.s_levels[level]): # if current action is achieveable (fulfill all the requirements)
                self.a_levels[level].add(na) # add this action to the tree

                # now take care of the parent-child relationship
                for ps in self.s_levels[level]: # for each parent literals
                    if ps in na.prenodes: # did this literal help to achieve current goal?
                        ps.children.add(na) 
                        na.parents.add(ps)
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set()) # add one more literal level
        for pa in self.a_levels[level-1]: # for parent actions
            for literal in pa.effnodes: # and for the result of each action parents
                if literal not in self.s_levels[level]: # if not already added
                    self.s_levels[level].add(literal)
                    pa.children.add(literal) 
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        n1_pos = set(node_a1.action.effect_add)
        n1_neg = set(node_a1.action.effect_rem)
        n2_pos = set(node_a2.action.effect_add)
        n2_neg = set(node_a2.action.effect_rem)
        if len((n1_pos.intersection(n2_neg)).union(n1_neg.intersection(n2_pos))) != 0:
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        n1_pre_pos = set(node_a1.action.precond_pos)
        n1_pre_neg = set(node_a1.action.precond_neg)
        n1_eff_pos = set(node_a1.action.effect_add)
        n1_eff_neg = set(node_a1.action.effect_rem)

        n2_pre_pos = set(node_a2.action.precond_pos)
        n2_pre_neg = set(node_a2.action.precond_neg)
        n2_eff_pos = set(node_a2.action.effect_add)
        n2_eff_neg = set(node_a2.action.effect_rem)

        a = (n1_pre_pos.intersection(n2_eff_neg)).union(n1_pre_neg.intersection(n2_eff_pos))
        b = (n2_pre_pos.intersection(n1_eff_neg)).union(n2_pre_neg.intersection(n1_eff_pos))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for p in node_a1.parents:
            if (p.mutex.intersection(node_a2.parents)):
                return True
        return False

        # why this doesn't work?!??!
        # for p in node_a1.prenodes:
        #     if (p.mutex.intersection(node_a2.prenodes)):
        #         return True
        #     else:
        #         pass
        # return False
