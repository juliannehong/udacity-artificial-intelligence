...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        # kb.clauses returns list of positive precondition
        # each action contains precond_pos and precond_neg
    ...
    def result(self, state: str, action: Action):
        ...
        # 1. positive init & won't be removed
        for f in current_state.pos:
            if f not in action.effect_rem:
                new_state.pos.append(f)

        # 2. add positivie effect to the new_state.pos if not already there
        for f in action.effect_add:
            if f not in new_state.pos:
                new_state.pos.append(f)

        # 3. negative init & won't be removed
        for f in current_state.neg:
            if f not in action.effect_add:
                new_state.neg.append(f)

        # 4. add negative effect to the new_state.neg if not already there
